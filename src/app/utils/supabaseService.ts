...
export async function diagnoseSupabaseSchema(): Promise<SchemaDiagnosis[]> {
  ...
  const checks = [
    ...
    {
      table: 'stations',
      query: () => supabase.from('stations').select('id, name').limit(1)
    },
    {
      table: 'station_requirements',
      query: () => supabase.from('station_requirements').select('id, station_id, training_name').limit(1)
    },
    {
      table: 'training_records',
      query: () => supabase.from('training_records').select('id, employee_number, employee_name, training_name, status, completion_date').limit(1)
    },
    {
      table: 'layout_positions',
      query: () => supabase.from('layout_positions').select('id, code, station_id, employee_number, coverage_type, line, shift').limit(1)
    }
  ];
...
// =========================================================================
// MÉTODOS CRUD VINCULADOS AL MÓDULO DE COMPETENCIAS Y VALIDACIÓN DE LINEPULSE
// =========================================================================

/**
 * Obtiene todas las estaciones del catálogo.
 */
export async function getSupabaseStations(): Promise<Station[]> {
  const { data, error } = await supabase
    .from('stations')
    .select('*')
    .order('name');
...
/**
 * Guarda o actualiza una estación en la base de datos.
 */
export async function saveSupabaseStation(station: Station): Promise<void> {
  const payload = await buildSafePayload('stations', {
    id: station.id,
    name: station.name,
...
/**
 * Elimina una estación del catálogo.
 */
export async function deleteSupabaseStation(id: string): Promise<void> {
  const { error } = await supabase
    .from('stations')
    .delete()
    .eq('id', id);
...
/**
 * Obtiene todos los requerimientos de entrenamiento por estación.
 */
export async function getSupabaseStationRequirements(): Promise<StationRequirement[]> {
  const { data, error } = await supabase
...
/**
 * Agrega un nuevo requerimiento a una estación.
 */
export async function saveSupabaseStationRequirement(req: StationRequirement): Promise<void> {
  const payload = await buildSafePayload('station_requirements', {
  ...
  const { error } = await supabase
    .from('station_requirements')
    .upsert(payload, { onConflict: 'station_id,training_name' });
  if (error) throw error;
}

/**
 * Elimina un requerimiento de entrenamiento específico de una estación.
 */
export async function deleteSupabaseStationRequirement(stationId: string, trainingName: string): Promise<void> {
  const { error } = await supabase
    .from('station_requirements')
    .delete()
...
/**
 * Obtiene todos los registros de entrenamiento importados de los empleados.
 */
export async function getSupabaseTrainingRecords(): Promise<TrainingRecord[]> {
  const { data, error } = await supabase
...
/**
 * Guarda o actualiza registros de entrenamiento en lote (batch), previniendo duplicados.
 */
export async function saveSupabaseTrainingRecords(records: TrainingRecord[]): Promise<void> {
  if (records.length === 0) return;

  // 1. Obtener registros existentes para cruzar por lote y no duplicar en base de datos.
  const existing = await getSupabaseTrainingRecords();
  
  // Clave de unicidad: empleado + curso
  const getUniqueKey = (empNo: string, training: string) => 
    `${empNo.trim().toLowerCase()}_${training.trim().toLowerCase()}`;
  ...
  for (const rec of records) {
    ...
    const payload = {
      employee_number: rec.employee_number.trim(),
      employee_name: rec.employee_name.trim(),
      training_name: rec.training_name.trim(),
    ...
    if (matched) {
      // Si el estado o la fecha cambiaron, lo actualizamos
      if (matched.status !== rec.status || matched.completion_date !== rec.completion_date) {
        toUpdate.push({
          id: matched.id,
          ...payload
        });
  ...
  // Ejecutar inserciones y actualizaciones en Supabase
  if (toInsert.length > 0) {
    const safePayloads = await Promise.all(toInsert.map(item => buildSafePayload('training_records', item)));
    const { error } = await supabase.from('training_records').insert(safePayloads);
    if (error) throw error;
  }

  if (toUpdate.length > 0) {
    const safePayloads = await Promise.all(toUpdate.map(item => buildSafePayload('training_records', item)));
    const { error } = await supabase.from('training_records').upsert(safePayloads, { onConflict: 'id' });
    if (error) throw error;
  }
}

/**
 * Obtiene la configuración de posiciones de layout de la planta.
 */
export async function getSupabaseLayoutPositions(): Promise<LayoutPosition[]> {
  const { data, error } = await supabase
...
/**
 * Guarda o actualiza una posición de layout (ej: POS01, POS02).
 */
export async function saveSupabaseLayoutPosition(position: LayoutPosition): Promise<void> {
  const payload = await buildSafePayload('layout_positions', {
    code: position.code,
    station_id: position.station_id,
    employee_number: position.employee_number || null,
    coverage_type: position.coverage_type || 'Normal',
    line: position.line || 'Línea 1',
    shift: position.shift || 'Turno 1',
    updated_at: new Date().toISOString()
  });
...
/**
 * Actualiza la asignación de un operador y tipo de cobertura en una posición específica.
 */
export async function updateSupabaseLayoutPositionAssignment(
  code: string,
  employeeNumber: string | null,
  coverageType: 'Normal' | 'Comedor' = 'Normal'
): Promise<void> {
  const payload = await buildSafePayload('layout_positions', {
    employee_number: employeeNumber,
...
/**
 * Elimina una posición del layout en Supabase.
 */
export async function deleteSupabaseLayoutPosition(code: string): Promise<void> {
  const { error } = await supabase
    .from('layout_positions')
    .delete()
    .eq('code', code);
