...
// Exámenes predeterminados oficiales generados a partir de examen_temas_lean.md
const defaultExams: Exam[] = officialExams as unknown as Exam[];
  ...
  useEffect(() => {
    try {
      ...
      if (savedExams) {
        ...
        const isOutdated = loadedExams.length < defaultExams.length || loadedExams.some(e => 
          !e.questions || e.questions.length < 10 || !e.questions.some(q => !!q.explanation)
        );
        if (isOutdated) {
          console.log('[LGB App] Actualizando banco de exámenes oficial desde exams.json...');
          loadedExams = defaultExams;
      ...
      if (isConnected) {
        ...
        try {
          ...
          if (dbExams.length === 0) {
            ...
            // Verificar si algún examen en Supabase está desactualizado o le faltan preguntas/explicaciones
            for (const defExam of defaultExams) {
              const exIndex = dbExams.findIndex((e: any) => e.courseId === defExam.courseId);
              if (exIndex === -1 || dbExams[exIndex].questions.length < 10 || !dbExams[exIndex].questions.some((q: any) => !!q.explanation)) {
                console.log(`[LGB App debug] Actualizando examen oficial '${defExam.courseId}' en Supabase...`);
                await saveSupabaseExam(defExam);
                if (exIndex !== -1) {
                  dbExams[exIndex] = defExam;
                } else {
                  dbExams.push(defExam);
                }
              }
            }
          }
